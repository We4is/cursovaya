...
function showError(msg) {
  ...
  if (w)
    w.innerHTML = `<div style="text-align:center;padding:60px;color:#888">
        <p>${msg}</p><br>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const node = [...xml.querySelectorAll("product")].find(
      (n) => n.querySelector("id")?.textContent === id,
    );
    ...
    document.querySelectorAll(".prod_tab").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll(".prod_tab")
          .forEach((b) => b.classList.remove("active"));
        document
          .querySelectorAll(".prod_tab_content")
          .forEach((c) => c.classList.remove("active"));
        btn.classList.add("active");
        document
          .getElementById(`tab_${btn.dataset.tab}`)
