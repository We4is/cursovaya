...
const catNames = {
    ...
    gainer: 'Гейнеры', collagen: 'Коллаген',
    magnesium: 'Магний', '': 'Все товары'
};
...
fetch('./products_all.xml')
    ...
    .then(text => {
        ...
        if (slMax) { slMax.max = maxPrice; slMax.value = maxPrice; priceMax = maxPrice; }
        if (slMin) { slMin.max = maxPrice; }
...
function showProducts() {
    ...
    if (title) title.textContent = currentSearch ? `Поиск: ${currentSearch}` : (catNames[currentCat] ?? 'Каталог');

    document.querySelectorAll('.cat_link').forEach(a => {
        a.classList.toggle('active', a.dataset.cat === currentCat);
    });

    const checkedBrands = [...document.querySelectorAll('#brand_list input:checked')].map(cb => cb.value);
    const onlyInStock   = document.getElementById('only_in_stock')?.checked ?? false;
    ...
    let result = products.filter(p => {
        ...
        if (currentSearch && !p.name.toLowerCase().includes(currentSearch.toLowerCase())
                          && !p.brand.toLowerCase().includes(currentSearch.toLowerCase())) return false;
    ...
    if (!shown.length) {
        container.innerHTML = '<p style="padding:40px;color:#888">Товары не найдены</p>';
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('sort_select')?.addEventListener('change', showProducts);
    document.getElementById('show_select')?.addEventListener('change', showProducts);

    document.getElementById('only_in_stock')?.addEventListener('change', showProducts);
